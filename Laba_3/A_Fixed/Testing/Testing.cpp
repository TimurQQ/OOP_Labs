...
namespace bigint_ns {
	...
	TEST(BigIntegerMethods, Operators) {
		...
		//Testing Plus
		{
			...
			// X > 0, Y > 0, X + Y < 1
			EXPECT_TRUE(a.Plus(b).DigitsCompare("979"));
			EXPECT_EQ(kPlus, a.Plus(b).sign()); // The answer is positive
			EXPECT_EQ(3, a.Plus(b).length());
			a.set_sign(kMinus);
			b.set_sign(kMinus);
			// X < 0, Y < 0, abs(X + Y) < 1
			EXPECT_TRUE(a.Plus(b).DigitsCompare("979"));
			EXPECT_EQ(kMinus, a.Plus(b).sign()); // The answer is negative
			EXPECT_EQ(3, a.Plus(b).length());
			// X > 0, Y > 0, X + Y >= 1 - Positive Overflow
			EXPECT_THROW(c.Plus(d), std::overflow_error);
			c.set_sign(kMinus);
			d.set_sign(kMinus);
			...
			EXPECT_EQ(kPlus, d.Plus(c).sign()); // The answer is positive
			EXPECT_EQ(48, d.Plus(c).length());
			d.set_sign(kMinus);
			c.set_sign(kPlus);
			// X > 0, Y < 0, X + Y < 0
			EXPECT_TRUE(c.Plus(d).DigitsCompare("731099200891801891910801801890810801810898098863"));
			EXPECT_EQ(kMinus, c.Plus(d).sign()); // The answer is negative
			EXPECT_EQ(48, c.Plus(d).length());
		}
		//Testing Minus : A - B = A + (-B) => We've already tested everything
		{
			...
			EXPECT_EQ(kPlus, f.Minus(e).sign()); // The answer is positive
			EXPECT_EQ(3, f.Minus(e).length());
			EXPECT_TRUE(f.Plus(minus_e).DigitsCompare("767"));
			EXPECT_EQ(kPlus, f.Plus(minus_e).sign()); // The answer is positive
			EXPECT_EQ(3, f.Plus(minus_e).length());
			// X + Y < 0
			EXPECT_TRUE(e.Minus(f).DigitsCompare("767"));
			EXPECT_EQ(kMinus, e.Minus(f).sign()); // The answer is negative
			EXPECT_EQ(3, e.Minus(f).length());
			EXPECT_TRUE(e.Plus(minus_f).DigitsCompare("767"));
			EXPECT_EQ(kMinus, e.Plus(minus_f).sign()); // The answer is negative
			EXPECT_EQ(3, e.Plus(minus_f).length());
		}
