...
namespace bigint_ns {
	...
	std::istream& operator >>(std::istream& st, BigInteger& a) {
		char s[BigInteger::kMaxSize_ + 1];
		st.getline(s, BigInteger::kMaxSize_ + 1, '\n');
		if (st) {
			bool is_signed = false;
			...
			a.set_digits(s + is_signed);
			a.digits_[BigInteger::kMaxSize_] = sign;
		}
		return st;
	}
	...
	bool BigInteger::IsCorrect(const char* number, bool& is_signed) noexcept {
